...
from utils.io_utils import paddle_gather
...
class UniterPreTrainedModel(nn.Layer):# {{{
    ...
    def init_weights(self, layer):
        ...
        if isinstance(layer, (nn.Linear, nn.Embedding)):
            ...
            layer.weight.set_value(paddle.ones(layer.weight.shape))
            layer.bias.set_value(paddle.zeros(layer.bias.shape))
...
class UniterTextEmbeddings(nn.Layer):# {{{
...
class UniterImageEmbeddings(nn.Layer):# {{{
    def __init__(self, config, img_dim):
        ...
        self.pos_layer_norm = BertLayerNorm(config.hidden_size, epsilon=1e-12)
